...
REQUIRED_SHEETS = {"KPIs", "KPI by ABC Class", "Part Level Breakdown"}
PART_HEADER = (
    "item",
    "item_description",
    "emr_class",
...
def load_workbook_data(path: Path) -> tuple[dict, list[list]]:
    ...
    try:
        ...
        if missing_sheets:
            raise ValueError(
                f"{path.name}: missing sheets {sorted(missing_sheets)}"
            )
        kpi_rows = list(workbook["KPIs"].iter_rows(values_only=True))
        part_sheet = workbook["Part Level Breakdown"]
        part_header = tuple(
            str(value or "").strip().lower()
            for value in next(part_sheet.iter_rows(values_only=True))[:6]
        )
        if part_header != PART_HEADER:
            raise ValueError(
                f"{path.name}: invalid Part Level Breakdown header {part_header}"
            )
        ...
        filename_month = re.search(
            r" - ([A-Za-z]+) \d{1,2} (\d{4})\.xlsx$",
            path.name,
        )
        if not filename_month:
            raise ValueError(f"{path.name}: invalid audit report filename")
        expected_month = datetime.strptime(
            f"{filename_month.group(1)} {filename_month.group(2)}",
            "%B %Y",
        )
        if target_month != expected_month:
            raise ValueError(
                f"{path.name}: filename month does not match workbook target month "
                f"{target_month:%B %Y}"
            )
...
def main() -> None:
    ...
    for workbook_path in workbook_paths:
        ...
        if month["key"] in parts_by_month:
            raise ValueError(f"Duplicate audit month: {month['key']}")
        months.append(month)
        parts_by_month[month["key"]] = parts

    months.sort(key=lambda month: month["key"])
    for previous, current in zip(months, months[1:]):
        previous_month = datetime.strptime(previous["key"], "%Y-%m")
        expected_year = previous_month.year + (previous_month.month == 12)
        expected_month_number = previous_month.month % 12 + 1
        expected_key = f"{expected_year:04d}-{expected_month_number:02d}"
        if current["key"] != expected_key:
            raise ValueError(
                f"Missing audit month between {previous['key']} and {current['key']}"
            )
    data = {
        "months": months,
